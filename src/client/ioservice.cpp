...
IoService::IoService(const commands_info::AuthInfo& ainf, const common::net::HostAndPort& server_host)
    : ILoopController(),
      ainf_(ainf),
...
void IoService::RequesRuntimeChannelInfo(stream_id_t sid) const {
  PrivateHandler* handler = static_cast<PrivateHandler*>(handler_);
  if (handler) {
...
void IoService::HandleStarted() {
  ignore_result(loop_thread_->Start());
}
